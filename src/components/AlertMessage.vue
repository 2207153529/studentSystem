<template  slot-scope="scope">
  <div>
    <el-button @click="getData" type="primary" style="margin-left: 16px;">
    查看详情
    </el-button>
    <el-drawer
    :visible.sync="drawer"
    :direction="direction"
    size="40%">
    <alert-message-childen :todoData="todoData"/>
    <div id="echarts"></div>
    </el-drawer>
  </div>
</template>
...
<script>
import AlertMessageChilden from './AlertMessageChilden.vue'
export default {
    name: "AlertMessage",
    components:{
        AlertMessageChilden
    ...
    watch: {
        // drawer(){
        //     if(this.drawer)this.draw()
        // }
    },
    methods: {
      getData(){
          this.drawer = true
          this.$bus.$once("getData", result=>{
            this.todoData = result
            var manNum = Number.parseInt(result.manNum)
            var girlNum = Number.parseInt(result.girlNum)
            var da = [{name: "男", value: manNum},
              {name: "女", value: girlNum}]
            this.echData = []
            da.forEach(p=>{
              this.echData.push(p)
            })
      ...
      drawChart() {
        ...
        var trafficWay = this.echData

        var data = [];
        var color = ['#ffa800', '#ff5b00', '#ff3000'];
